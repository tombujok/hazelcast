...
public class IndexesTest {
    ...
    public void testAndWithSingleEntry() throws Exception {
        ...
        for (int i = 0; i < 20000; i++) {
            ...
            indexes.saveEntryIndex(new QueryEntry(ss, toData(i), i, employee, Extractors.empty()), null);
        }
        int count = 1000;
    ...
    public void testIndex2() throws Exception {
        Indexes indexes = new Indexes(ss, Extractors.empty());
        indexes.addOrGetIndex("name", false);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(1), 1, new Value("abc"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(2), 2, new Value("xyz"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(3), 3, new Value("aaa"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(4), 4, new Value("zzz"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(5), 5, new Value("klm"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(6), 6, new Value("prs"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(7), 7, new Value("prs"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(8), 8, new Value("def"), Extractors.empty()), null);
        indexes.saveEntryIndex(new QueryEntry(ss, toData(9), 9, new Value("qwx"), Extractors.empty()), null);
    ...
    private void shouldReturnNull_whenQueryingOnKeys(Indexes indexes) {
        for (int i = 0; i < 50; i++) {
            ...
            indexes.saveEntryIndex(new QueryEntry(ss, toData(i), i, null, Extractors.empty()), null);
        }
