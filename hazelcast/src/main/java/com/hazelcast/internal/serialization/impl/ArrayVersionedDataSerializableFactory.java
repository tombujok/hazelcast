...
public final class ArrayVersionedDataSerializableFactory implements VersionedDataSerializableFactory {

    private final ConstructorFunction<Integer, Versioned>[] constructors;
    private final int len;

    public ArrayVersionedDataSerializableFactory(ConstructorFunction<Integer, Versioned>[] ctorArray) {
    ...
    @Override
    public Versioned create(int typeId, byte version) {
        if (typeId >= 0 && typeId < len) {
            ConstructorFunction<Integer, Versioned> factory = constructors[typeId];
